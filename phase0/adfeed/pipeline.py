"""AdFeed AI — 核心流水线编排器 v2.1（多国原生语种版本）
...
def run(excel_path: str = None, countries: list[str] = None,
    ...
    # ── Step 3: 逐行 + 逐国处理 ──
    print(f"\n[3/6] Processing {total} SKU x {len(countries)} countries...")
    ...
    for i, (_, row) in enumerate(df.iterrows(), 1):
        sku = str(row.get("SKU", f"SKU{i:04d}"))
        title = str(row.get("标题", ""))
        desc = str(row.get("描述", ""))
        category = str(row.get("分类", ""))
        # 代发卖家无分类：从标题关键词快速推断品类
        if not category or category.lower() in ("nan", "none", ""):
            category = guess_category_from_title(title)
        brand = str(row.get("品牌", row.get("brand", "")))
        material = str(row.get("材质", ""))
        color = str(row.get("颜色", ""))
        size = str(row.get("尺码", row.get("size", "")))
        image_url = str(row.get("图片链接", ""))
        additional_images_raw = str(row.get("附加图片", row.get("additional_images", "")))
        ...
        if row_dedup and not force:
            print(f"SKIP (dedup, last: {row_dedup['last_processed_at'][:19]})")
            dedup_skipped += 1
            tokens_saved += 5000 * len(countries)
            existing = lookup(sku)
            if existing:
                results.append({
                    "sku": sku, "original_title": title,
                    "optimized_titles": existing.get("optimized_titles", {}),
                    "gpc_code": existing.get("gpc_code", ""),
                    "gpc_path": existing.get("gpc_path", ""),
        ...
        if classification["action"] == "price_inventory_update":
            ...
            print(f"PRICE/INV UPDATE (0 Token) → ${classification['price_usd']}")
            price_updated += 1
            tokens_saved += 5000 * len(countries)
            existing = classification["existing"]
            results.append({
        ...
        # 多国语批量生成标题
        print(f"[{len(needs_countries)} countries]", end=" ")
        ...
        ai_country_calls += len(needs_countries)
        tokens_estimated += 5000 * len(needs_countries)
        ...
        if total_violations > 0:
            print(f"⚠{total_violations}v", end=" ")
        else:
            print("OK", end=" ")

        # 产品属性推断：gender / age_group / item_group_id
    ...
    summary = {
        ...
        "total_country_calls": ai_country_calls,
        "ai_full_clean": ai_processed,
        "partial_reclean": partial_reclean,
        "skipped_old": skipped_old,
        "price_updated": price_updated,
        "dedup_skipped": dedup_skipped,
        "compliance_warnings": compliance_warnings,
        "watermark_flagged": watermark_flagged,
    ...
    print(f"  {total} SKU x {len(countries)} countries = {ai_country_calls} AI calls")
    print(f"  AI cleaned: {ai_processed} full + {partial_reclean} partial")
    print(f"  Skipped: {skipped_old} old + {price_updated} price/inv updates + {dedup_skipped} dedup")
    print(f"  Tokens est: {tokens_estimated:,}  |  Saved: {tokens_saved:,}")
